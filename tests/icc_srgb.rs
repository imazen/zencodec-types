...
use zencodec::icc_profile_is_srgb;
...
fn ghostscript_srgb() {
    ...
    if let Ok(data) = std::fs::read(path) {
        assert!(icc_profile_is_srgb(&data), "ghostscript srgb.icc should match");
    }
}
...
// ── SourceColor::is_srgb() integration ─────────────────────────────────

#[test]
fn source_color_cicp_srgb() {
    use zencodec::{Cicp, SourceColor};
